...
;SoundPlay, %A_Temp%\@DIRUE_TEMPFILES\sounds\scream_final.wav ;cant be in this script because this script deletes this files root directory
;}
;play_start_sound_func()
...
; Program Titles
ProgWinTitle1 = DeadIslandRiptideUltimateEdition_By_FireEyeEian
...
cleanup()
if (FileExist("%A_Temp%\@DIRUE_TEMPFILES\")) {
	failed_1()
	}
else{
	allgood()
...
allgood(){
	MsgBox,,All gucci,Temporary files deleted`, Thanks for using DIRUE by FireEyeEian :)`n                                   (App will close now in 4 seconds),4
	Self_destruct_on_reboot()
}

failed_2(){
	MsgBox, failed to delete tempfiles? `n`nshit brok??`n`n(maybe try manually deleting @DIRUE_TEMPFILES folder in temp folder located in C:\Users\{username}\AppData\Local\Temp)
	Self_destruct_on_reboot()
}
